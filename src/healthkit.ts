...
/**
 * Creates a reverse map from filename to metric name
 */
const FILE_TO_METRIC_MAP: Record<string, string> = Object.fromEntries(
  Object.entries(METRIC_TO_FILE_MAP).map(([metric, file]) => [file, metric])
);

/**
 * Sorts keys in all healthkit JSON files according to their schema order
 */
export async function sortKeysInHealthkitFiles(
  basePath: string,
  reader: ReaderFunction,
  writer: WriterFunction
): Promise<{ success: boolean; message: string }> {
  const files = Object.values(METRIC_TO_FILE_MAP);
  ...
  for (const fileName of files) {
    try {
      ...
      const metricName = FILE_TO_METRIC_MAP[fileName];
      
      if (!metricName) {
        errors.push(`Unknown metric for file: ${fileName}`);
        continue;
      }
      ...
      // Reorder keys in each metric according to schema
      const reorderedMetrics = existingData.metrics.map((metric) =>
        reorderKeysBySchema(metric, metricName)
      );
      ...
      results.push(`Sorted keys in ${fileName} (${reorderedMetrics.length} entries)`);
    } catch (error) {
  ...
  return {
    success: true,
    message: `✅ Successfully sorted keys in all healthkit files!\n\n${results.map(r => `- ${r}`).join("\n")}`,
  };
}
