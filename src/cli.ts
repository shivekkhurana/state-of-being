...
import { ingestHealthDataFromIssue, ingestHealthDataFromExport, parseHealthData, sortAllHealthkitFiles } from "@src/healthkit";
...
program
  ...
  .action(async () => {
    try {
      ...
      // Sort all healthkit files
      const result = await sortAllHealthkitFiles(
        writer,
        reader,
        config.healthkitFolderPath
      );
